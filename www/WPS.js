...
OpenLayers.WPS = OpenLayers.Class({
    ...
    parseGetCapabilities: function (resp) {
        ...
        for (var i = 0; i < operationsMetadata.length; i++) {
            ...
            var get = null;
            try {
               get = OpenLayers.Format.XML.prototype.getAttributeNS(getNode,this.xlinkNS, "href");
            } catch(e) {}
            var postNode = OpenLayers.Format.XML.prototype.getElementsByTagNameNS(operationsMetadata[i],this.owsNS, "Post")[0];
            var post = null;
            try {
               post = OpenLayers.Format.XML.prototype.getAttributeNS(postNode,this.xlinkNS, "href");
            } catch(e) {}
    ...
    parseDescribeComplexPuts: function(dom){
	 ...
	 if (cmplxData.length > 0) {
            ...
            var tmp = formatsNode.getElementsByTagName("MimeType");
            if (tmp.length == 0) {
               tmp = formatsNode.getElementsByTagName("ows:MimeType");
            }
            tmp = tmp[0];
            var frmts = tmp.firstChild.nodeValue;
            formats.push(frmts);
            ...
            for (var i = 0; i < supportedFormats.length; i++) {
                var format = supportedFormats[i].getElementsByTagName("MimeType");
                if (format.length == 0) {
                   format = supportedFormats[i].getElementsByTagName("ows:MimeType");
                }
                format = format[0].firstChild.nodeValue;
    ...
    executePost : function(identifier) {
        ...
        if (process.async == null) {
           data = data.replace("$STORE_AND_STATUS$","false");
        } else {
        ...
        for (var i  = 0; i < process.inputs.length; i++ ) {
            ...
            var tmpl = "";
            var format = null;
            try { format = input.formats[0]; } catch(e) { format = input.format; }
            var formatStr ="";
        ...
        for (var i = 0; i < process.outputs.length; i++) {
            ...
            if (output.CLASS_NAME.search("Complex")>-1) {
                ...
                var format = null;
                try { format = output.formats[0]; } catch(e) { format = output.format; }
                var formatStr ="";
                if (format) {
                    if (format.mimeType) {
                        formatStr += " mimeType=\""+format.mimeType+"\"";
	...
	if (reference.length > 0) {
            var tmp = OpenLayers.Format.XML.prototype.getAttributeNS(reference[0],"", "href");
	    if (tmp.length == 0) {
		    tmp = reference[0].getAttribute("xlink:href");
	    }
            output.setValue(tmp);
        }
